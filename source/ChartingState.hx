	...
	override function create()
	{
		...
		if (PlayState.SONG != null)
		{
			...
			else
			{
				...
				_song = Song.conversionChecks(Song.loadFromJson(PlayState.SONG.songId, ""));
			}
		}
		else
		{
		...
		try
		{
			*/
			...
			if (playClaps)
			{
				...
				{
					if (note.strumTime <= Conductor.songPosition && note.strumTime > Conductor.songPosition - 50 && !claps.contains(note) && FlxG.sound.music.playing)
					{
						claps.push(note);
